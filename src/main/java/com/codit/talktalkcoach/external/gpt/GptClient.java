...
public class GptClient {
    ...
    private String getCategoryGuide(SpeechCategory c) {
        return switch (c) {
            ...
            case DEBATE -> """
                    ...
                    ⚠️ DEBATE 카테고리 채점 전 반드시 확인하세요 (불일치 페널티):
                    1. 논제(찬성/반대 입장이 있는 주장)가 명시적으로 제시되었는가?
                       → 없으면 logicScore -15점 페널티
                    2. 수치·사례·인용 등 구체적 근거가 있는가?
                       → 없으면 logicScore -10점 페널티
                    3. 상대 의견 인정 또는 반론 대응 구조가 있는가? (MIDDLE_1_2 이상)
                       → 없으면 structureScore -10점 페널티
                    4. 위 3가지 중 2개 이상 없다면:
                       → 이 스피치는 DEBATE 유형과 맞지 않습니다.
                       → logicFeedback에 반드시 명시: "이 스피치는 토론보다 발표에 가까운
                          형태입니다. 토론에서는 논제 제시와 구체적 근거가 필수입니다."
                       → 최종 점수는 반드시 82점 이하로 제한하세요.
                    """;
        };
    }
    ...
    private String getStructureRubric(SpeechCategory c) {
        return switch (c) {
            case PRESENTATION -> """
                    ⚠️ structureScore 채점 전 존재 여부를 먼저 확인하세요:
                    - 도입부(인사말, 주제 예고, 흥미 유발)이 명시적으로 있는가?
                      → 없으면 이 세부 항목 0점
                    - 정리부(요약 또는 마무리)가 명시적으로 있는가?
                      → 없으면 이 세부 항목 0점
                    - 도입부와 정리부 둘 다 없다면 structureScore 최대 52점으로 제한하세요.
    ...
    private String getFeedbackFormat(TargetLevel level, String field) {
        ...
        if (field.equals("overallFeedback")) {
            if (isShort)  return "\"<2문장만. 칭찬 1문장 + 개선 1문장. 각 문장 30자 이내. 금지어(어휘/논리/구조/전달력/유창성/발표/연설/논증) 절대 사용 금지. 초등학교 1학년이 바로 이해할 수 있는 말로만.>\"";
            if (isMedium) return "\"<3문장. 칭찬 1 + 개선 1 + 응원.>\"";
            return "\"<3~4문장. 칭찬 1 + 개선 1 + 응원. 과정/성취 중심 칭찬.>\"";
        }
        if (isShort)  return "\"<반드시 1문장만. 띄어쓰기 포함 30자 이내. 금지어(어휘/논리/구조/전달력/유창성/발표/연설/논증) 절대 사용 금지. 초등학교 1학년이 바로 이해할 수 있는 말로만.>\"";
